...
app_pid() {
  echo `ps -fe | grep org.apache.spark.deploy.master.Master | grep -v grep | tr -s " "|cut -d" " -f2`
}
...
start() {
  ...
  then
    ...
    /bin/su $APP_USER -c 'nohup {{ansible_env.HOME}}/spark-1.2.0-bin-hadoop2.4/sbin/start-master.sh 2>&1 > {{ansible_env.HOME}}/log-spark-master.out &'
    status
  fi
  return 0
}
