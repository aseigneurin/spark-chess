...
#APP_BASE is the location of the app
export APP_BASE={{ansible_env.HOME}}
 
#APP_USER is the default user of the app
export APP_USER={{ansible_env.USER}}
...
app_pid() {
  echo `ps -fe | grep org.apache.spark.deploy.worker.Worker | grep -v grep | tr -s " "|cut -d" " -f2`
}
...
start() {
  ...
  then
    ...
    /bin/su $APP_USER -c 'nohup {{ansible_env.HOME}}/spark-1.2.0-bin-hadoop2.4/bin/spark-class org.apache.spark.deploy.worker.Worker spark://{{master_hostname}}:7077 --cores {{spark_worker_cores}} --memory {{spark_worker_memory}} 2>&1 > {{ansible_env.HOME}}/log-spark-worker.out &'
    status
  fi
  return 0
}
