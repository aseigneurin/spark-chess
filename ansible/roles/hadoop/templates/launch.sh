...
app_pid() {
  echo `ps -fe | grep Dproc_{{item}} | grep -v grep | tr -s " "|cut -d" " -f2`
}
...
start() {
  ...
  then
    ...
    /bin/su $APP_USER -c 'nohup {{ansible_env.HOME}}/hadoop-2.6.0/bin/hdfs {{item}} 2>&1 > {{ansible_env.HOME}}/log-{{item}}.out &'
    status
  fi
  return 0
}
